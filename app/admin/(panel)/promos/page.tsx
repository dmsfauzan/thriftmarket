...
export default function AdminPromos() {
  ...
  async function add() {
    const r = await fetch("/api/admin/promos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code, label, discount: Number(discount), maxUses: maxUses || null, expiresAt: exp || null }) });
    if (!r.ok) return alert((await r.json()).error ?? "Gagal");
    setCode(""); setLabel(""); setDiscount("10"); setMaxUses(""); setExp("");
    refetch();
  }
    ...
    <div className="space-y-6">
      ...
      <Card title="Tambah Kupon">
        <div className="grid gap-3 sm:grid-cols-5">
          ...
          <input type="number" value={maxUses} onChange={(e) => setMaxUses(e.target.value)} placeholder="Max Uses" className="rounded-xl border border-slate-200 px-4 py-2 text-sm" />
        ...
        <div className="space-y-2">
          {promos.map((p: any) => (
            <div key={p.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 p-4">
              ...
              <div className="flex-1"><p className="font-mono text-sm font-bold">{p.code} — {p.discount}% {p.isActive ? "" : "(nonaktif)"}</p><p className="text-xs text-slate-400">{p.label ?? "-"} • {p.usageCount}{p.maxUses ? `/${p.maxUses}` : ""} terpakai • {p.expiresAt ? `exp ${new Date(p.expiresAt).toLocaleDateString("id-ID")}` : "tanpa batas"}</p></div>
